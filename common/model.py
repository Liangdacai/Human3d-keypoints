...
class TemporalModelBase(nn.Module):
    ...
    def __init__(self, num_joints_in, in_features, num_joints_out,
        ...
        self.shrink_kps = nn.Conv1d(channels, num_joints_out * 3, 1)
        self.shrink_heap = nn.Conv1d(channels, 3, 1)
    ...
    def forward(self, x):
        ...
        x0,x1 = self._forward_blocks(x)
        
        x0 = x0.permute(0, 2, 1)
        x0 = x0.view(sz[0], -1, self.num_joints_out, 3)

        x1 = x1.permute(0, 2, 1)
        x1 = x1.view(sz[0], -1, 1, 3)
...
class TemporalModel(TemporalModelBase):
    ...
    def _forward_blocks(self, x):
        ...
        x_kps = self.shrink_kps(x)
        x_heap = self.shrink_heap(x)
        return x_kps, x_heap
